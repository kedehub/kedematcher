...
from kedehub.matching.idmatching import strip_accents

blacklists_dir = Path(__file__).parents[1] / "blacklists"


def _read_blacklist(name: str) -> List[str]:
    with gzip.open(str(blacklists_dir / ("%s.csv.gz" % name)), "rt", encoding='utf-8') as f:
